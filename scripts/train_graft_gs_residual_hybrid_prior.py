...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train GRAFT-GS residual hybrid prior."
    )
    parser.add_argument(
...
def build_prior(config: dict[str, Any]) -> GaussianPrimitiveResidualHybridAR:
...
@torch.no_grad()
def compute_geometry_normalizer(
    *,
    ...
    for batch in tqdm(loader, desc="computing geometry normalizer", leave=False):
        batch = move_batch_to_device(batch, device)

        with autocast(device_type=device.type, enabled=mixed_precision):
            primitives = tokenizer.encode(batch.images)
            geometry = flatten_primitives(primitives)[..., :6]

        flat = geometry.reshape(-1, 6).float()
    ...
    return PrimitiveNormalizer(mean=mean.detach().cpu(), std=std.detach().cpu())
...
def encode_targets(
    *,
    ...
    geometry = normalizer.normalize(geometry)
    feature_codes = codebook.assign(features)
...
def train_one_epoch(
    *,
    model: GaussianPrimitiveResidualHybridAR,
    tokenizer: GaussianSplatTokenizer,
    codebook: ResidualFeatureCodebook,
    normalizer: PrimitiveNormalizer,
    loader: torch.utils.data.DataLoader[ImageBatch],
    ...
    total_loss = 0.0
    total_geometry_nll_loss = 0.0
    total_geometry_mean_loss = 0.0
    total_feature_code_loss = 0.0
    num_batches = 0

    progress = tqdm(
        loader, desc=f"train GRAFT-GS residual hybrid prior epoch {epoch}", leave=False
    )
    ...
    for batch in progress:
        ...
        with torch.no_grad():
            geometry, feature_codes = encode_targets(
                tokenizer=tokenizer,
                codebook=codebook,
        ...
        with autocast(device_type=device.type, enabled=mixed_precision):
            output = model(geometry, feature_codes, batch.labels)
            loss = output.loss
        ...
        total_geometry_nll_loss += output.geometry_nll_loss.item()
        total_geometry_mean_loss += output.geometry_mean_loss.item()
        total_feature_code_loss += output.feature_code_loss.item()
        num_batches += 1

        progress.set_postfix(
            loss=f"{output.loss.item():.4f}",
            geom=f"{output.geometry_mean_loss.item():.4f}",
            code=f"{output.feature_code_loss.item():.4f}",
        )

    return {
        "loss": total_loss / num_batches,
        "geometry_nll_loss": total_geometry_nll_loss / num_batches,
        "geometry_mean_loss": total_geometry_mean_loss / num_batches,
        "feature_code_loss": total_feature_code_loss / num_batches,
    }
...
def generate_images(
    *,
    prior: GaussianPrimitiveResidualHybridAR,
    tokenizer: GaussianSplatTokenizer,
    codebook: ResidualFeatureCodebook,
    normalizer: PrimitiveNormalizer,
    batch_size: int,
...
def main() -> None:
    ...
    normalizer = compute_geometry_normalizer(
        tokenizer=tokenizer,
        loader=train_loader,
        device=device,
        mixed_precision=mixed_precision,
    )
    torch.save(normalizer.state_dict(), run_dir / "geometry_normalizer.pt")

    print("GRAFT-GS residual hybrid prior training")
    print("---------------------------------------")
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"val geom mse {val_metrics['geometry_mean_loss']:.4f} | "
            f"val code ce {val_metrics['feature_code_loss']:.4f}"
        )
        ...
        extra = {
            "geometry_normalizer": normalizer.state_dict(),
        ...
        if (
            ...
            save_generated_samples(
                ...
                geometry_temperature=config["sampling"]["geometry_temperature"],
